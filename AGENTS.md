...
- **306 mechanics** (`BASE_MECHS` array, M001-M306) across **72 researched games**: FF7 Rebirth,
  ...
  **Trails in the Sky** (M284-M285): a recipe book filled only by tasting (58 dishes, first eaten
  ...
  **Wave 4** — **Ni no Kuni** (M289, g098): merit awards, where every errand and bounty pays stamps
  toward cards spent on passive abilities in three tiers that must each be bought out, the cheapest
  tier being movement (a jump, world-map and ship speed, foraging) and the dearest award an
  eight-card +100 HP and MP for Oliver; and the Crypt Casino (g098, a 16-row table), whose prize
  ...
  **Fire Emblem: Three Houses** (M293, g101, g102, M241 sharpened): the Professor Level made concrete,
  ten ranks that raise Activity Points from 1 to 10 and the greenhouse to five seeds, fed by nearly
  every activity including the free ones; the greenhouse (M293), whose permanent stat booster is
  rolled at planting; fishing (g101), with its shadows and professor experience per fish tabled; and
  tea time (g102), which pays support and +1 Charm to both for a Great or Perfect finish.
  **Yakuza: Like a Dragon** (M294, g103): Part-Time Hero, a city quest board whose support and rescue
  quests pay points toward four rank-ups, each collected as gear (at 15, 70, 150 and 300 points), while
  challenge quests pay personality stats; and Can Quest (g103), whose Eco Points buy 100,000-point
  manuals and boosters at Kan-san's shop. The business payouts and Sujimon came back THIN. Wave 5 is
  complete, and with it the rollout: all 15 games.
...
- **`VERBS`** — the 15 discovery verbs from GAME_PROMPT_V2 §3, keyed by display name
  (like CATS). Mechanics carry an optional `verbs:[...]`; **91 of 306 rows are tagged**
  (110 tags). **Every tag is justified in `docs/verbs.md`**, where it quotes a verbatim span
...
- **`LINEAGES`** — chains of mechanics that share one design shape, `{name, ids, best,
  counter?, note}`; **12 lineages** covering 67 mechanics. Rendered under the Design Pillars
...
- **`RETIRED`** (added 2026-09-15) — the only way a row leaves the file. `{g081:"g006"}` in the
  data region maps an id to the row that absorbed it: FF6's Dragon's Neck Coliseum had two cards,
  and g081's lesson went into g006. A retired id is never reused. `migrateRetired(s, RETIRED)`,
  called by `normalizeStore` on every load and import, moves a favourite, a board placement or a
  pillar judgement to the successor unless it has its own, fills the successor's override gaps (a
  pinned video, a rating, notes) and deletes the retired key; it is a frozen migration that reads
  only the map (`scripts/mygame.test.mjs`). The entry that retires an id lists it in `retired`.
...
- **Facets** (added 2026-09-13; the owner: "I want everything clickable") — every platform,
  ...
  **`facetSources(g)` is the ONE list of where a facet comes from** — `gf.plat`, `also`,
  ...
  and `engine`; the categories in `wpcats` that `CATEGORY_FACETS` names (below); the row's `year`
  and `status`; and the row's own `dev` (split on " / ") only when
  ...
  **`CATEGORY_FACETS`** (added 2026-09-13, from the owner's audit of unused research data) turns
  chosen Wikipedia categories into facets: `{kind: {label: [category, …]}}` over each row's raw
  `wpcats`, matched through `facetNorm`. It adds the kinds `theme`, `feature` and `award`, and may
  also widen a vocabulary kind. A label under `genre` must be a `FACET_VOCAB.genre` canonical, so
  "Turn-based role-playing video games" lists under the GameFAQs genre Turn-Based instead of
  starting a near-identical second filter. Categories that restate an existing facet (platforms,
  years, companies, composers, engines, series) are deliberately left unmapped, as are media
  adaptations. Several categories share a label only when every game in each is that thing: a
  theme crossed with a genre ("Science fantasy role-playing video games") folds into the theme,
  while two different things (an award and its Game of the Year) never share one.
  The validator holds every category in the table to one some game carries (read from the raw
...
- **`refs`** — optional provenance on mechanic AND minigame rows:
  ...
  **231 of 306 mechanics and 93 of 104 minigames carry `refs`**, and every row without them
...
- **Script-owned game-row fields** (added 2026-09-07; every one of the 72 rows carries `gf`,
  `cover` and `wp` as of that day, all 72 carry `wd`, `infobox` and `wpcats` since 2026-09-13, 55 carry `digest`)
  ...
  with `plat` and `at` required and never without `wp`. `wd` is the article's Wikidata item
  and `wpcats` every visible category of it, stored raw (the "Category:" prefix dropped, sorted,
  deduped), both from the same run and both never without `wp`. Custom games never carry any of these, so every renderer must degrade
  without them.
...
- `scripts/fetch_infobox.mjs [--write] [--only "<title>"…] [--note "<text>"] [--today <date>]`
  ...
  `CHANGES` as a `games` entry, widening a same-day one. A second call per row (`action=query`,
  `prop=categories|pageprops`, hidden categories excluded) records `wd` and `wpcats`. It refuses
