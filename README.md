...
good**. It holds 293 mechanics across 72 games, 102 minigames with their actual reward
...
| | |
|---|---|
| **293 mechanics** | Each with how it works, its reward loop (`do X → get Y → feels good because Z`), and notes on adapting it |
| **102 minigames** | 57 carry concrete reward tables — the exact item at the exact threshold, not "prizes and gil" |
...
- **My Game board** — everything you mark *Yes* collects into a workspace. You place each
  ...
  counts every pillar's support and how much you have not judged yet, and names the
  directors, designers, writers and composers who recur across the games you picked
...
```bash
node scripts/fetch_infobox.mjs --write
```
