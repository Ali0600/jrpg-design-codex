...
- **298 mechanics** (`BASE_MECHS` array, M001-M298) across **72 researched games**: FF7 Rebirth,
...
- Mechanic row shape: `{id:"M001", game, name, cat, how, loop, rating, want, notes}`
  - `cat` must be a key of CATS. Every base row ships `want:""` and `rating:0`, and the validator
    ...
    their browser.
  - IDs are sequential: mechanics M001-M298, minigames g001-g104, skipping a retired id. Continue the
...
- **`VERBS`** — the 15 discovery verbs from GAME_PROMPT_V2 §3, keyed by display name
  (like CATS). Mechanics carry an optional `verbs:[...]`; **88 of 298 rows are tagged**
...
- **`refs`** — optional provenance on mechanic AND minigame rows:
  ...
  **200 of 298 mechanics and 82 of 103 minigames carry `refs`**, and every row without them
